<?php
$pageTitle = 'Add Property — Elite Estates Admin';
        ...
        if (!$title || !$price) {
            ...
            $stmt = $db->prepare(
                "INSERT INTO properties
                 (title,location,price,type,beds,baths,sqft,garage,year_built,description,img,status,featured,created_at)
                 VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,NOW())"
            );
            $stmt->execute([$title,$location,$price,$type,$beds,$baths,$sqft,$garage,$year_built,$description,$img,$status,$featured]);
            $msg = 'Property added successfully!';
            $title=$location=$description=$img='';
            $price=$beds=$baths=$sqft=$garage=$year_built=0;
            $featured=0;
        }
    }
}
  ...
  <form method="POST" enctype="multipart/form-data">
    ...
    <div class="form-grid">
      <div class="form-group form-floating" style="grid-column: span 2;">
        <input type="text" name="title" id="title" class="form-control" required placeholder=" " value="<?= htmlspecialchars($title ?? '') ?>">
        <label for="title">Property Title *</label>
      </div>

      <div class="form-group form-floating">
        <input type="number" name="price" id="price" class="form-control" required min="0" step="100" placeholder=" " value="<?= $price ?: '' ?>">
      ...
      <div class="form-group form-floating">
        <select name="type" id="type" class="form-control">
          <?php foreach(['Villa','Penthouse','Mansion','Estate','Apartment','Townhouse'] as $t): ?>
            <option value="<?= strtolower($t) ?>" <?= (isset($type) && $type === strtolower($t)) ? 'selected' : '' ?>><?= $t ?></option>
          <?php endforeach; ?>
        </select>
      ...
      <div class="form-group form-floating" style="grid-column: span 2;">
        <input type="text" name="location" id="location" class="form-control" placeholder=" " value="<?= htmlspecialchars($location ?? '') ?>">
    ...
    <div class="form-grid">
      <div class="form-group form-floating">
        <input type="number" name="beds" id="beds" class="form-control" min="0" placeholder=" " value="<?= $beds ?: '' ?>">
        <label for="beds">Bedrooms</label>
      </div>
      
      <div class="form-group form-floating">
        <input type="number" name="baths" id="baths" class="form-control" min="0" placeholder=" " value="<?= $baths ?: '' ?>">
        <label for="baths">Bathrooms</label>
      </div>

      <div class="form-group form-floating">
        <input type="number" name="sqft" id="sqft" class="form-control" min="0" placeholder=" " value="<?= $sqft ?: '' ?>">
        <label for="sqft">Square Footage</label>
      </div>

      <div class="form-group form-floating">
        <input type="number" name="garage" id="garage" class="form-control" min="0" placeholder=" " value="<?= $garage ?: '' ?>">
        <label for="garage">Garage Spaces</label>
      </div>

      <div class="form-group form-floating">
        <input type="number" name="year_built" id="year_built" class="form-control" min="1800" max="2030" placeholder=" " value="<?= $year_built ?: '' ?>">
    ...
    <div class="form-group" style="margin-top: 1rem;">
      <label class="custom-checkbox">
        <input type="checkbox" name="featured" value="1" <?= (isset($featured) && $featured) ? 'checked' : '' ?>>
    ...
    <div class="form-group">
      <label style="display: block; font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.1em; color: var(--platinum); margin-bottom: 1rem;">Primary Image Upload</label>
      ...
      <div class="upload-zone" id="dropZone" onclick="document.getElementById('img_file').click()">
        ...
        <div class="upload-text">
          <div style="font-family: var(--font-heading); font-size: 1.4rem; color: var(--warm-white); margin-bottom: 0.5rem;">Drop your image here</div>
      ...
      <div class="form-floating">
        <input type="url" name="img_url" id="imgUrl" class="form-control" placeholder=" " value="<?= htmlspecialchars($img ?? '') ?>">
    ...
    <div class="form-group form-floating" style="margin-top: 2rem;">
      <textarea name="description" id="description" class="form-control" placeholder=" "><?= htmlspecialchars($description ?? '') ?></textarea>
    ...
    <div style="display: flex; gap: 1.5rem; margin-top: 4rem;">
      <button type="submit" class="btn-luxury" style="padding: 1.2rem 3rem;">
        Commit Property Details <i data-lucide="arrow-right" style="width: 18px; height: 18px;"></i>
      </button>
    </div>
  </form>
</div>
